...
docker run --network host --name ansible_${RANDOM} -e ANSIBLE_SSH_CONTROL_PATH=/dev/shm/ssh_ctl_%%h_%%p_%%r --rm -v /root/.ssh:/root/.ssh -v `pwd`:$docker_basedir -w $docker_basedir -i$tty congcong126/ansible:2.9.18 ansible-playbook -f 10 --ssh-common-args="$ANSIBLE_SSH_COMMON_ARGS" -i $1 system_init/system_init.yml
